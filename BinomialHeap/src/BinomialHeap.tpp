...
void BinomialHeap<T>::Merge(BinomialHeap &otherHeap) {
    if (this->head_.get() != nullptr && otherHeap.head_.get() != nullptr) {
        ...
        std::shared_ptr<Node> current_Heap1_node = this->head_;
        std::shared_ptr<Node> current_Heap2_node = otherHeap.head_;
        if (this->head_.get()->degree_ < otherHeap.head_.get()->degree_) {
            Heap->head_ = this->head_;
            current_Heap1_node = current_Heap1_node.get()->right_sibling_;
        } else {
            Heap->head_ = otherHeap.head_;
            current_Heap2_node = current_Heap2_node.get()->right_sibling_;
        }
        std::shared_ptr<Node> current_Heap_node = Heap->head_;
        while (current_Heap1_node.get() != nullptr && current_Heap2_node.get() != nullptr) {
            if (current_Heap1_node.get()->degree_ < current_Heap2_node.get()->degree_) {
                current_Heap_node.get()->right_sibling_ = current_Heap1_node;
                current_Heap1_node.get()->left_sibling_ = current_Heap_node;
                current_Heap1_node = current_Heap1_node.get()->right_sibling_;
            } else {
                current_Heap_node.get()->right_sibling_ = current_Heap2_node;
                current_Heap2_node.get()->left_sibling_ = current_Heap_node;
                current_Heap2_node = current_Heap2_node.get()->right_sibling_;
            }
            current_Heap_node = current_Heap_node.get()->right_sibling_;
        }
        if (current_Heap1_node.get() == nullptr) {
            while (current_Heap2_node.get() != nullptr) {
                current_Heap_node.get()->right_sibling_ = current_Heap2_node;
                current_Heap2_node.get()->left_sibling_ = current_Heap_node;
                current_Heap2_node = current_Heap2_node.get()->right_sibling_;
                current_Heap_node = current_Heap_node.get()->right_sibling_;
        ...
        if (current_Heap2_node.get() == nullptr) {
            while (current_Heap1_node.get() != nullptr) {
                current_Heap_node.get()->right_sibling_ = current_Heap1_node;
                current_Heap1_node.get()->left_sibling_ = current_Heap_node;
                current_Heap1_node = current_Heap1_node.get()->right_sibling_;
                current_Heap_node = current_Heap_node.get()->right_sibling_;
        ...
        current_Heap_node = Heap->head_;
        std::shared_ptr<Node> pre_current_Heap_node;
        pre_current_Heap_node.reset();
        while (current_Heap_node.get()->right_sibling_.get() != nullptr) {
            if (current_Heap_node.get()->degree_ == current_Heap_node.get()->right_sibling_.get()->degree_) {
                if (current_Heap_node.get()->key_ < current_Heap_node.get()->right_sibling_.get()->key_) {
                    std::shared_ptr<Node>temporary = current_Heap_node.get()->right_sibling_.get()->right_sibling_;
                    current_Heap_node.get()->right_sibling_.get()->right_sibling_ = current_Heap_node.get()->left_child_;
                    if (current_Heap_node.get()->left_child_.get() != nullptr) {
                        current_Heap_node.get()->left_child_.get()->left_sibling_ = current_Heap_node.get()->right_sibling_;
                    }
                    current_Heap_node.get()->right_sibling_.get()->left_sibling_.reset();
                    current_Heap_node.get()->left_child_ = current_Heap_node.get()->right_sibling_;
                    current_Heap_node.get()->left_child_.get()->parrent_ = current_Heap_node;
                    current_Heap_node.get()->right_sibling_ = temporary;
                    if (temporary.get() != nullptr) {
                        temporary.get()->left_sibling_ = current_Heap_node;
                    }
                } else {
                    if(pre_current_Heap_node.get() != nullptr) {
                        pre_current_Heap_node.get()->right_sibling_ = current_Heap_node.get()->right_sibling_;
                        current_Heap_node.get()->right_sibling_.get()->left_sibling_ = pre_current_Heap_node;
                    }
                    else {
                        Heap->head_ = current_Heap_node.get()->right_sibling_;
                    }
                    std::shared_ptr<Node>temporary = current_Heap_node;
                    current_Heap_node = current_Heap_node.get()->right_sibling_;
                    current_Heap_node.get()->left_sibling_ = temporary.get()->left_sibling_;
                    temporary.get()->right_sibling_ = current_Heap_node.get()->left_child_;
                    if (current_Heap_node.get()->left_child_.get() != nullptr) {
                        current_Heap_node.get()->left_child_.get()->left_sibling_ = temporary;
                    }
                    current_Heap_node.get()->left_child_ = temporary;
                    if (current_Heap_node.get()->left_child_.get() != nullptr) {
                        current_Heap_node.get()->left_child_.get()->left_sibling_.reset();
                    }
                    temporary.get()->parrent_ = current_Heap_node;
                }
                ++current_Heap_node.get()->degree_;
            }
            else {
                pre_current_Heap_node = current_Heap_node;
                current_Heap_node = current_Heap_node.get()->right_sibling_;
            }
        }
